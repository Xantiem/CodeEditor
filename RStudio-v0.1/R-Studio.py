...
class RStudio(tk.Frame):
    ...
    def new_file(self):
        self.filename = fd.asksaveasfilename(defaultextension=".r", filetypes=[("R Script", "*.r"), ("All files", "*.*")])
        self.basepath = os.path.dirname(self.filename)
        print(self.filename, self.basepath)
    ...
    def editor(self, parent):
        ...
        self.writer.insert("1.0", "Open or create a new file to get started...\nDon't forget png(filename=\"\")\n\nVersion: 0.1 - Absolute paths and universal file locations added")
...
root = tk.Tk()
root.title("R-STDio - Made in China Edition")

screen_width = root.winfo_screenwidth()
screen_height = root.winfo_screenheight()
root.geometry(f"{screen_width}x{screen_height}+0+0")

rstudio = RStudio(root)
root.mainloop()